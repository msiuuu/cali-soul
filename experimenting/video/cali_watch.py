...
usage:
    ...
    python3 cali_watch.py save <frame_path> [--label "note"]  # save a keeper permanently
"""
...
def save_frame(frame_path: Path, label: str = None) -> Path:
    SAVED_DIR.mkdir(parents=True, exist_ok=True)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    stem = f"{timestamp}_{label.replace(' ', '_')}" if label else timestamp
    dest = SAVED_DIR / f"{stem}.jpg"
    shutil.copy2(frame_path, dest)
    print(f"[cali_watch] saved → {dest}")
    return dest
...
def main():
    ...
    parser.add_argument("--save", action="store_true", help="also copy all extracted frames to saved/")
    ...
    if args.save:
        ...
        for i, f in enumerate(frames):
            timestamp = args.start + (i * args.interval)
            label = f"{args.label}_t{int(timestamp)}" if args.label else f"t{int(timestamp)}"
            save_frame(f, label)
